...
1. **Single View（単一表示）**
   - 従来の単一コード表示
   - 現在のコードのマーブルパターンを全画面表示
   - コード選択・追加時のデフォルト表示

2. **Timeline View（タイムライン表示）**
   - コード進行を左から右へ流れるタイムラインとして表示
   - 2つのサブモードを持つ：
...
│   ├── KeySelector（キー選択）
│   ├── BPMInput（BPM入力）
│   └── HueRotationInput（色相回転調整）
├── VisualizationCanvas（メイン表示エリア）
│   ├── MarbleRenderer（マーブル描画）
│   └── ParticleSystem（カラースプレー）
...
│   └── VolumeSlider
├── ChordInput（コード入力UI）
│   ├── ChordPalette（コード選択パレット）
│   └── ChordSequence（追加済みコード列）
└── StatusDisplay（現在のコード情報表示）
```
