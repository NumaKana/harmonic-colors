...
#### #22 [Phase 2] 小節ベース表示UI
**説明**: コード進行を小節単位で視覚的に区切って表示

**タスク**:
- [ ] ChordSequenceコンポーネントのUIを小節グリッド表示に変更
- [ ] 4拍子を基準とした小節区切り線の表示
- [ ] コードの長さに応じた幅の調整
- [ ] 小節番号の表示（オプション）
...
#### #23 [Phase 2] コード長さ選択機能
**説明**: 各コードの長さ（拍数）をユーザーが選択可能に

**タスク**:
- [ ] ChordPaletteにコード長さ選択UIを追加
- [ ] 全音符（4拍）、2分音符（2拍）、4分音符（1拍）、8分音符（0.5拍）を選択可能に
- [ ] 選択した長さをChordオブジェクトのdurationに反映
- [ ] ChordSequenceで長さを視覚的に表示
...
#### #24 [Phase 2] Three.js VisualizationCanvas実装
**説明**: CSS gradientからThree.js/WebGLへの移行

**タスク**:
- [ ] Three.jsとReact Three Fiberの統合
- [ ] 既存のCSS gradientをシェーダーで再現
- [ ] 基本的なカメラとシーン設定
- [ ] パフォーマンス最適化（60fps維持）
- [ ] フォールバック処理（WebGL非対応環境）

**完了条件**:
- Three.jsで現在のCSS gradientと同じ見た目を再現
- 60fpsで動作

---

#### #25 [Phase 2] コード進行タイムライン表示
**説明**: 左から右へ流れる色遷移のタイムライン表示

**タスク**:
- [ ] Three.jsでタイムライン用のシーンを構築
- [ ] 各コードを色付きのセグメントとして表現
- [ ] 再生位置のインジケーター（縦線）
- [ ] スムーズなスクロールアニメーション
- [ ] 再生モードとプレビューモードの切替
...
**完了条件**:
- 再生中に左から右へ色が流れる
- プレビューモードで全体の色遷移を確認可能

---

#### #9 [Phase 2] Three.js統合とシェーダーセットアップ (旧)
...
#### #26 [Phase 4] セクション別key指定機能（転調対応）
**説明**: コード進行を複数のセクションに分割し、各セクションごとに異なるkeyを指定可能に

**タスク**:
- [ ] Sectionタイプの定義
- [ ] App.tsxのデータ構造変更（Chord[] → Section[]）
- [ ] セクション管理UI（追加・削除・編集・並び替え）
- [ ] セクションごとのkey選択
- [ ] ChordPalette、ChordSequence、PlaybackControlsの更新
- [ ] VisualizationCanvasのセクション変更対応
...
**完了条件**:
- セクションの追加・削除・編集が可能
- 各セクションごとに異なるkeyを設定可能
- セクション間の転調が再生時に正しく反映される
